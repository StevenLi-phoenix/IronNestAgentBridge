...
- 弹药成本(征用点, **实价以本局清单为准**): 侦察弹(STAR/SMK, 通常2点)比杀伤弹便宜一个
  ...
  **化学弹**(若本局可购, 机制以实测为准): PHGN光气(约10点, 半径620m)名义每点覆盖全场最高,
  但预计只对**人员**有效——步兵集群首选, 对工事/装甲/建筑别指望; TEAR催泪(约8点, 半径750m,
  **零杀伤**)是面积压制/驱散手段, 逼步兵离开阵地用, 绝不能当杀伤弹。两者半径巨大,
  **用前必须entities_near排查友军/平民**; 统帅部若禁用化学武器则绝对服从。
  例外: 统帅部明确限制弹种时从其指令。
