...
- 弹种选择: armour=0的单体目标(步兵/无甲车辆)**默认用LE**(比HE便宜, 精确弹): 有
  entityId或可信坐标时一律LE; 仅当瞄点存疑、需要容错半径时才升HE(半径250m vs LE 150m)。
...
- 弹药成本(征用点, **实价以本局清单为准**): 快照每轮给出**征用点余额**, 购买/出膛事件也附
  实时余额。**fire/requisition_card在下单时自动查账**: 弹价(加上队列在排任务将先扣的钱)超出
  余额会直接被拒并说明缺口——收到"征用点不足"就等在排任务结清、取消低价值任务或换便宜弹种,
  不要原样重试; 余额紧张时优先留够反炮兵应对的钱(杀伤弹或MoveZone)。侦察弹(STAR/SMK, 通常2点)比杀伤弹便宜一个
