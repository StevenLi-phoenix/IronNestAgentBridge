...
- 友军安全: 弹着点爆炸半径内有友军/平民(role含Ally/Spotter/civilian)时fire会拒绝并警告。
  例外: **SMK烟幕弹无杀伤, 完全豁免友军检查**——对友军阵位/撤退路线放烟遮蔽是合法战术。
...
- 弹药成本(征用点, **实价以本局清单为准**): 侦察弹(STAR/SMK, 通常2点)比杀伤弹便宜一个
  ...
  **化学弹**(若本局可购): PHGN光气(约10点, 半径620m)——**仅对"处于被压制状态"的人员
  ...
  正被压制时**默认不选它**, 步兵照常用HE/HCHE。**WP白磷**(约10点, 半径750m, 零直接杀伤,
  推测为大范围**压制/燃烧**弹, 待实测): 若确认能压制, 则WP压制→PHGN收割构成对大股步兵的
  标准组合技(20点清750m圈, 但需两发两炮位); 单独使用价值存疑。化学/燃烧弹半径巨大,
