...
- **定序连击**: FCS按优先级带调度, 带内同优先级任务的执行顺序由引擎按转炮效率自行优化,
  **不保证入队先后**。需要严格先后的连击(如 炮兵→FDC): **先只排第1发, 收到它的
  shell_fired(出膛)事件后再排第2发**——用事件把顺序锁死; 不同优先级(如92和91)只保证
  高优先先上炮, 不保证先出膛。
...
""";
    ...
    private static readonly (string Key, string Intel)[] MapIntelTable =
    {
        ...
        ("最终收割", "起手式(开局照做): 本图开局即自由开火, 但**先找AA再放开打**。" +
                    "关键机制: **AA被打掉之前, ScoutPlane飞机侦察和STAR照明全部无效**——" +
                    "严禁在AA存活时购买ScoutPlane或打STAR(纯浪费), FO是唯一可用的侦察手段。" +
                    ...
                    "(击毁FDC会暂停反炮击计时)。**顺序必须锁死**: 先只排第1发, 等它shell_fired出膛后再排FDC那发" +
                    "(同时在队FCS不保证先后), 期间不插其他任务; 之后是约**2分钟装填期**, 装填完成再开下一组, " +
                    "任何时刻在队+在炮任务不超过2发; " +
