...
- 弹药成本(征用点, **实价以本局清单为准**): 侦察弹(STAR/SMK, 通常2点)比杀伤弹便宜一个
  ...
  **化学弹**(若本局可购): PHGN光气(约10点, 半径620m, **仅对人员有效**)——步兵/人员集群的
  面杀伤首选(620m半径10点, 人员目标性价比碾压HCHE), 对工事/装甲/载具/建筑**无效**,
  混合目标群仍用HE/HCHE; 半径巨大, **用前必须entities_near排查友军/平民**,
  统帅部若禁用化学武器则绝对服从。TEAR催泪(约8点, 半径750m, 零杀伤, **使区域内单位显形**)
  ——侦察弹, 750m是全弹种最大的单发揭示范围: 大片可疑区一发揭开时用它;
  常规试探性盲射仍用STAR(2点)更省, TEAR留给"确信有目标群、要一次看全"的场合。
  例外: 统帅部明确限制弹种时从其指令。
