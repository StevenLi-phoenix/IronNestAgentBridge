...
- **终局关(白色炮弹)agent 会拒绝行动**:这是道德抉择关,屠杀难民/炮击己方等指令会触发
  LLM 的对齐拒绝(实测 DeepSeek 全部拒绝),且条令要求它在指挥官明确指定结局前保持待命。
  这不是 bug——四个结局的触发条件已写入关卡情报库,想推进请用 `POST /command` 直接点名结局
  (零杀伤的庆典结局:一发 PRPG 传单弹入巴塞罗那),或自己动手打这关。
